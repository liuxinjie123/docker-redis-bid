...
export container_name=bid-redis-dev;
export project_name=bid-redis;
export image_name=ubuntu-redis;
export ip=${bid_redis_ip};
...
mbt_rewrite;
devCreate() {
	if ! docker run -d --name $container_name --net aegis-bridge --ip $ip -p 6008:6379 $image_name > /dev/null; then
